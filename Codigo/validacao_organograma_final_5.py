...
CAMINHO_PPTX = r"C:\Users\augus\OneDrive\Documentos\Automação\Organograma_Atacado_IPC_Jul26.pptx"
CAMINHO_FENIX = r"C:\Users\augus\OneDrive\Documentos\Automação\Validação - 15.06.2026 - Base Fênix.xls"
CAMINHO_SAIDA = r"C:\Users\augus\OneDrive\Documentos\Automação\relatorio_organograma.csv"
...
def get_gestor_alt_text(shape):
    ...
    if not descr:
        return None
    m = re.match(r'^Gestor\s*=\s*(.+)', descr.strip(), re.IGNORECASE)
    if not m:
        return None
...
def get_vinculo_alt_text(shape):
    ...
    if not descr:
        return None
    m = re.match(r'^V[ií]nculo\s*=\s*(.+)', descr.strip(), re.IGNORECASE)
    if not m:
        return None
    return re.sub(r'\s+', ' ', m.group(1)).strip()
...
def extract_names_vinculo(pptx_path, known_names=None):
    ...
    def find_sibling_area_label(shapes):
        """Se, no MESMO grupo de uma lista de FUNCIONÁRIOS, existir só mais
        uma caixa de texto que não é nome de ninguém (ex: 'MINAS GERAIS',
        'NORDESTE 2') - trata como um nível extra de área pra essa lista.
        Só aplica quando há exatamente 1 lista de funcionários nesse grupo,
        pra não arriscar casar o rótulo errado quando há 2+ listas juntas."""
        rosters, labels = [], []
        for shp in shapes:
            ...
            if NAME_DICT:
                for l in lines:
                    l_clean = re.sub(r'\(.*?\)', '', l).strip()
                    m_, _ = line_matches_known_name(norm(l_clean))
                    if m_:
                        has_name = True
                        break
            if not has_name and lines:
                labels.append(' '.join(lines))
        if len(rosters) == 1 and len(labels) == 1:
            return labels[0]
        return None

    def walk(shapes, slide_num):
        sibling_area_label = find_sibling_area_label(shapes)
        for shp in shapes:
            if shp.shape_type == MSO_SHAPE_TYPE.GROUP:
...
def build_person_map(pptx_path, known_names):
    """Para CADA caixa de cabeçalho (nome+área) do deck inteiro, sem nenhuma
    geometria: guarda (área própria, gestor próprio) por pessoa. Simples
    parse de shape isolado - não depende de aninhamento nem posição."""
    set_name_dictionary(known_names)
    p = Presentation(pptx_path)
        ...
        for shp in shapes:
            ...
            for l in lines:
                ...
                if matched and nome_line is None:
                    ...
                    area_lines.append(l_clean)
            if nome_line is None:
                continue

            gestor_alt = get_gestor_alt_text(shp)
            key = norm(nome_line)
            area_text = ' '.join(area_lines) if area_lines else None
            existing = person_map.get(key, {})
            person_map[key] = dict(
                nome=nome_line,
                area=area_text or existing.get('area'),
...
def main():
    ...
    for _, r in fenix.iterrows():
        ...
        if m is None:
            ...
            status_gestor = 'N/A'
            area_pptx, status_area = None, 'N/A'
            status_area_rigoroso, status_area_tolerante = 'N/A', 'N/A'
        else:
            ...
            if own_info and own_info.get('area'):
                chain = chain + [own_info['area']]
            # rótulo de área irmão da própria lista de funcionários dela (ex:
            # "MINAS GERAIS" ao lado de "FUNCIONÁRIOS:") - o nível mais
            # específico, direto da caixa onde ela está listada
            area_extra = m.get('area_extra')
            if pd.notna(area_extra) and area_extra:
                chain = chain + [area_extra]
            ...
            if not chain and len(fenix_check_values) == 0:
                ...
                status_area_tolerante = 'OK'
            elif not chain:
                status_area_rigoroso = 'SEM AREA NO PPTX'
                status_area_tolerante = 'SEM AREA NO PPTX'
            else:
                # RIGOROSA: todo nível que o Fênix espera precisa aparecer na
                # cadeia do PPT (só tolera nível A MAIS na cadeia, tipo um
                # rótulo de produto que o Fênix não rastreia)
                ...
                # TOLERANTE: cada nível da cadeia bate com algo no Fênix, com
                # até 1 nível sem correspondência (de qualquer um dos lados)
                ...
                status_area_tolerante = 'OK' if niveis_batidos >= 1 and sem_par <= 1 else 'AREA DIVERGENTE'
            status_area = status_area_rigoroso
        ...
        results.append(dict(
            nome=r['Nome'], vinculo_fenix=r['Vínculo'], vinculo_pptx=vinc_pptx, status_vinculo=status,
            ...
            area_pptx=area_pptx, status_area_rigoroso=status_area_rigoroso, status_area_tolerante=status_area_tolerante,
    ...
    print(res.status_area_tolerante.value_counts())
    print()
